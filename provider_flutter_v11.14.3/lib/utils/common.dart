...
import 'package:lotech/components/html_widget.dart';
import 'package:lotech/components/new_update_dialog.dart';
import 'package:lotech/main.dart';
import 'package:lotech/utils/configs.dart';
import 'package:lotech/utils/constant.dart';
import 'package:lotech/utils/image_crop_screen.dart';
...
Future<void> setSaveSubscription({
  ...
  if (appConfigurationStore.isInAppPurchaseEnable) {
    ...
    await appStore
        .setActiveRevenueCatIdentifier(subscription.activePlanRevenueCatIdentifier.isNotEmpty ? subscription.activePlanRevenueCatIdentifier : getStringAsync(ACTIVE_IN_APP_PURCHASE_IDENTIFIER));
  }
}
...
void share({required String url, required BuildContext context}) {
  if (Platform.isIOS) {
    try {
      ...
      SharePlus.instance.share(ShareParams(text: url, sharePositionOrigin: box!.localToGlobal(Offset.zero) & box.size));
    } catch (e) {
